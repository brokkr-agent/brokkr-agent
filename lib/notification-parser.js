 ...
 * Uses XML conversion since JSON fails on binary data in plists
...
export function parseNotificationData(data) {
  ...
  try {
    ...
    // Convert to XML using plutil (JSON fails on binary data in plists)
...
/**
 * Parse XML plist to extract key values
 * Simple parser for notification plists - extracts string values
...
function parseXmlPlist(xml) {
  ...
  // Extract top-level string values: <key>name</key><string>value</string>
  ...
  // Extract boolean true values
  const truePattern = /<key>([^<]+)<\/key>\s*<true\/>/g;
  while ((match = truePattern.exec(xml)) !== null) {
    result[match[1]] = true;
  }

  // Extract real/float values
  const realPattern = /<key>([^<]+)<\/key>\s*<real>([^<]+)<\/real>/g;
  while ((match = realPattern.exec(xml)) !== null) {
    result[match[1]] = parseFloat(match[2]);
  }

  // Extract integer values
  const intPattern = /<key>([^<]+)<\/key>\s*<integer>([^<]+)<\/integer>/g;
  while ((match = intPattern.exec(xml)) !== null) {
    result[match[1]] = parseInt(match[2], 10);
  }

  // Look for nested 'req' dict and extract its values
  const reqMatch = xml.match(/<key>req<\/key>\s*<dict>([\s\S]*?)<\/dict>/);
  if (reqMatch) {
    const reqXml = reqMatch[1];
    const req = {};

    const reqStringPattern = /<key>([^<]+)<\/key>\s*<string>([^<]*)<\/string>/g;
    while ((match = reqStringPattern.exec(reqXml)) !== null) {
      req[match[1]] = match[2];
    }
