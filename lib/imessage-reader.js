 ...
 * @returns {Promise<Array<{id: number, text: string|null, timestamp: number, sender: string}>>}
 */
export async function getRecentMessages(phoneNumber, limit = 10) {
  ...
  try {
    ...
    // Strip the + from phone number for LIKE query
    ...
    const rows = db.prepare(query).all(`%${normalizedNumber}%`, limit);
