...
const COMMAND_CATEGORIES = {
  TASKS: {
    title: 'TASKS',
    commands: ['claude', 'c'],
    description: 'Run Claude tasks'
  },
  SESSIONS: {
    title: 'SESSIONS',
    commands: ['sessions', 'status', 's'],
    description: 'Manage sessions and status'
  },
  SCHEDULING: {
    title: 'SCHEDULING',
    commands: ['schedule'],
  ...
  SKILLS: {
    title: 'SKILLS',
    commands: ['research', 'r', 'x', 'twitter', 'github', 'gh', 'email', 'youtube', 'yt'],
    description: 'Pre-built skills'
  },
  HELP: {
    title: 'HELP',
    commands: ['help', 'h', '?'],
...
function formatCategorizedHelp(registry) {
  ...
  for (const [categoryKey, category] of Object.entries(COMMAND_CATEGORIES)) {
    ...
    for (const cmd of commands) {
      if (shownCommands.has(cmd.name)) continue;

      const cmdCategory = getCategoryForCommand(cmd.name);
      if (cmdCategory === categoryKey) {
        categoryCommands.push(cmd);
        shownCommands.add(cmd.name);
      }
    }
