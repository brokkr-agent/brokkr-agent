...
export function registerBuiltinCommands(registry = getDefaultRegistry()) {
  ...
  // /help (alias: h) - Show available commands
  registry.register(
  ...
  // /questions (alias: ?) - View pending approval requests from contacts (hidden from help)
  registry.register(
    CommandFactory.internal({
      name: 'questions',
  ...
  // /digest - View daily digests of pending/resolved questions (hidden from help)
  registry.register(
    CommandFactory.internal({
      name: 'digest',
